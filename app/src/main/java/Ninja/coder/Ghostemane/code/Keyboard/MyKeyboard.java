...
public class MyKeyboard extends Keyboard {
   ...
   public static class MyKeyboards extends Key {
        ...
        public MyKeyboards(Resources resources, Row row, int i, int i2, XmlResourceParser xmlResourceParser) {
	...
	public Key createKeyFromXml(Resources resources, Row row, int i, int i2, XmlResourceParser xmlResourceParser) {
		...
		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(ApplicationLoader.getContext());
		...
		if (ramin == Configuration.ORIENTATION_LANDSCAPE) {
			if (PreferenceManager.getDefaultSharedPreferences(ApplicationLoader.getContext()).getBoolean("Height", true)) {
