...
public class FileManagerUtils {
  ...
  private void Extract_icon() {
    if (FileUtil.isExistFile(iconFile)) {
      ...
      var dialogCompat =
          new ProgressDialogCompat(compat, ProgressDialog.THEME_DEVICE_DEFAULT_DARK);
      ...
      dialogCompat.show();
      dialogCompat.setProgressDrawable(new ColorDrawable(MaterialColors.getColor(compat,ColorAndroid12.colorOnSurface,0)));
  ...
  private MaterialShapeDrawable custom() {
  	var myShap = new MaterialShapeDrawable(
      ShapeAppearanceModel.builder().setAllCorners(CornerFamily.CUT,20f)
      .build()
    );
    myShap.setFillColor(ColorStateList.valueOf(MaterialColors.getColor(compat,ColorAndroid12.Back,0)));
    myShap.setStroke(0.5f,ColorStateList.valueOf(MaterialColors.getColor(compat,ColorAndroid12.colorOnSurface,0)));
    return myShap;
  }
}
