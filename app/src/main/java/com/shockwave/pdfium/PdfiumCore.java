...
public class PdfiumCore {
    ...
    public void renderPage(
        ...
        synchronized (lock) {
            try {
                ...
                nativeRenderPage(
                        doc.mNativePagesPtr.get(pageIndex),
                        surface,
                        mCurrentDpi,
                        startX,
                        startY,
     ...
     * Page must be opened before rendering.
     *
     * <p>Supported bitmap configurations:
     *
     * <ul>
     ...
     * Page must be opened before rendering.
     *
     * <p>For more info see {@link PdfiumCore#renderPageBitmap(PdfDocument, Bitmap, int, int, int,
     * int, int)}
     */
    public void renderPageBitmap(
            PdfDocument doc,
    ...
    private void recursiveGetBookmark(
            List<PdfDocument.Bookmark> tree, PdfDocument doc, long bookmarkPtr) {
     ...
     * @param rotate    page orientation: 0 (normal), 1 (rotated 90 degrees clockwise), 2 (rotated 180
     *                  degrees), 3 (rotated 90 degrees counter-clockwise)
     * @param pageX     X value in page coordinates
     * @param pageY     Y value in page coordinate
     * @return mapped coordinates
     */
    public Point mapPageCoordsToDevice(
            PdfDocument doc,
            int pageIndex,
            int startX,
            int startY,
    ...
    /**
     * @return mapped coordinates
     * @see PdfiumCore#mapPageCoordsToDevice(PdfDocument, int, int, int, int, int, int, double,
     * double)
     */
    public RectF mapRectToDevice(
        ...
        Point leftTop =
                mapPageCoordsToDevice(
                        doc, pageIndex, startX, startY, sizeX, sizeY, rotate, coords.left, coords.top);
        Point rightBottom =
                mapPageCoordsToDevice(
                        doc, pageIndex, startX, startY, sizeX, sizeY, rotate, coords.right, coords.bottom);
