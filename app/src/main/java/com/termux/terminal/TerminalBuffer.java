...
public final class TerminalBuffer {
   ...
   * @param screenRows the height of just the screen, not including the transcript that holds lines
   *     that have scrolled off the top of the screen.
   */
  ...
  public String getSelectedText(
      int selX1, int selY1, int selX2, int selY2, boolean joinBackLines, boolean joinFullLines) {
   ...
   * columns does not change or the rows expand (that is, it only works when shrinking the number of
   * rows).
   *
    ...
    if (newColumns == mColumns && newRows <= mTotalRows) {
      ...
      if (shiftDownOfTopRow > 0 && shiftDownOfTopRow < mScreenRows) {
        ...
        if (shiftDownOfTopRow != actualShift) {
          // The new lines revealed by the resizing are not all from the transcript. Blank the below
          // ones.
      ...
      mScreenFirstRow =
          (mScreenFirstRow < 0) ? (mScreenFirstRow + mTotalRows) : (mScreenFirstRow % mTotalRows);
      ...
      // Blank lines should be skipped only if at end of transcript (just as is done in the "fast"
      // resize), so we
      ...
      for (int externalOldRow = -oldActiveTranscriptRows;
        ...
        if (oldLine == null || (!(!newCursorPlaced && cursorAtThisRow)) && oldLine.isBlank()) {
          ...
          // After skipping some blank lines we encounter a non-blank line. Insert the skipped blank
          // lines.
        ...
        for (int i = 0; i < lastNonSpaceIndex; i++) {
          ...
          int codePoint =
              (Character.isHighSurrogate(c)) ? Character.toCodePoint(c, oldLine.mText[++i]) : c;
  ...
  public void scrollDownOneLine(int topMargin, int bottomMargin, long style) {
    ...
    // Copy the fixed mScreenRows-bottomMargin lines one line down so that they remain on screen in
    // same
    // position:
