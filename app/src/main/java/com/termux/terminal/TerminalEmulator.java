...
public final class TerminalEmulator {
   ...
   * depends on the current top margin setting. The cursor cannot move outside of the margins. When
   * DECOM is reset, the home cursor position is at the upper-left corner of the screen. The
   ...
   * characters received when the cursor is at the right border of the page appear at the beginning
   * of the next line. Any text on the page scrolls up if the cursor is at the end of the scrolling
   * region. If the DECAWM function is reset, then graphic characters received when the cursor is at
   * the right border of the page replace characters already on the page."
   */
  ...
  public void sendMouseEvent(int mouseButton, int column, int row, boolean pressed) {
    ...
    if (mouseButton == MOUSE_LEFT_BUTTON_MOVED
      ...
      mSession.write(
          String.format("\033[<%d;%d;%d" + (pressed ? 'M' : 'm'), mouseButton, column, row));
    } else {
      ...
      if (!out_of_bounds) {
        byte[] data = {
          '\033', '[', 'M', (byte) (32 + mouseButton), (byte) (32 + column), (byte) (32 + row)
        };
  ...
  private void processByte(byte byteToProcess) {
    if (mUtf8ToFollow > 0) {
      if ((byteToProcess & 0b11000000) == 0b10000000) {
        ...
        // "If the converter encounters an ill-formed UTF-8 code unit sequence which starts with a
        // valid first
        // byte, but which does not continue with valid successor bytes (see Table 3-7), it must not
        // consume the
          ...
          case ESC_CSI_DOLLAR:
            ...
            switch (b) {
              case 'v': // ${CSI}${SRC_TOP}${SRC_LEFT}${SRC_BOTTOM}${SRC_RIGHT}${SRC_PAGE}${DST_TOP}${DST_LEFT}${DST_PAGE}$v"
                ...
                int bottomSource =
                    Math.min(
                        Math.max(getArg(2, mRows, true) + effectiveTopMargin, topSource), mRows);
                int rightSource =
                    Math.min(
                ...
                int destinationLeft =
                    Math.min(getArg(6, 1, true) - 1 + effectiveLeftMargin, mColumns);
              ...
              case 'z': // ${CSI}$${TOP}${LEFT}${BOTTOM}${RIGHT}$z"
                ...
                // "Pch can be any value from 32 to 126 or from 160 to 255. If Pch is not in this
                // range, then the
                ...
                if ((fillChar >= 32 && fillChar <= 126) || (fillChar >= 160 && fillChar <= 255)) {
                  ...
                  for (int row = top - 1; row < bottom; row++)
                    for (int col = left - 1; col < right; col++)
                      if (!selective
                          || (TextStyle.decodeEffect(mScreen.getStyleAt(row, col))
              ...
              case 't': // "${CSI}${TOP}${LEFT}${BOTTOM}${RIGHT}${ATTRIBUTES}$t"
                ...
                // mode (DECOM)".
                int top =
                    Math.min(getArg(0, 1, true) - 1, effectiveBottomMargin) + effectiveTopMargin;
                int left =
                    Math.min(getArg(1, 1, true) - 1, effectiveRightMargin) + effectiveLeftMargin;
                int bottom =
                    Math.min(getArg(2, mRows, true) + 1, effectiveBottomMargin - 1)
                        + effectiveTopMargin;
                int right =
                    Math.min(getArg(3, mColumns, true) + 1, effectiveRightMargin - 1)
                        + effectiveLeftMargin;
                if (mArgIndex >= 4) {
          ...
          case ESC_CSI_ARGS_ASTERIX:
            ...
            if (b == 'x' && (attributeChangeExtent >= 0 && attributeChangeExtent <= 2)) {
              ...
              setDecsetinternalBit(
                  DECSET_BIT_RECTANGULAR_CHANGEATTRIBUTE, attributeChangeExtent == 2);
            } else {
              unknownSequence(b);
            }
            break;
      ...
      case (byte) '\\': // End of ESC \ string Terminator
        {
          ...
          if (dcs.startsWith("$q")) {
            ...
            // Request Termcap/Terminfo String. The string following the "q" is a list of names
            // encoded in
            // hexadecimal (2 digits per character) separated by ; which correspond to termcap or
            // terminfo key
            // names.
            // Two special features are also recognized, which are not key names: Co for termcap
            // colors (or colors
            // for terminfo colors), and TN for termcap name (or name for terminfo name).
            // xterm responds with DCS 1 + r P t ST for valid requests, adding to P t an = , and the
            // value of the
            // corresponding string that xterm would send, or DCS 0 + r P t ST for invalid requests.
            // The strings are
            ...
            for (String part : dcs.substring(2).split(";")) {
              if (part.length() % 2 == 0) {
                ...
                mClient.logError(
                    LOG_TAG, "Invalid device termcap/terminfo name of odd length: " + part);
              }
            }
          } else {
  ...
  private void doCsiQuestionMark(int b) {
    switch (b) {
      ...
      case 'n': // Device Status Report (DSR, DEC-specific).
        switch (getArg0(-1)) {
          case 6:
            // Extended Cursor Position (DECXCPR - http://www.vt100.net/docs/vt510-rm/DECXCPR).
            // Page=1.
            mSession.write(
                String.format(Locale.US, "\033[?%d;%d;1R", mCursorRow + 1, mCursorCol + 1));
            break;
          default:
            finishSequence();
            return;
  ...
  public void doDecSetOrReset(boolean setting, int externalBit) {
    ...
    switch (externalBit) {
      ...
      case 3: // Set: 132 column mode (. Reset: 80 column mode. ANSI name: DECCOLM.
        ...
        // (FIXME: Should only do this if the 95 DECSET bit (DECNCSM) is set, and if changing
        // value?):
        // Sets the left, right, top and bottom scrolling margins to their default positions, which
        // is important for
  ...
  private void doCsiBiggerThan(int b) {
    switch (b) {
      case 'c': // "${CSI}>c" or "${CSI}>c". Secondary Device Attributes (DA2).
        // Originally this was used for the terminal to respond with "identification code, firmware
        // version level,
        // and hardware options" (http://vt100.net/docs/vt510-rm/DA2), with the first "41" meaning
        // the VT420
        // terminal type. This is not used anymore, but the second version level field has been
        // changed by xterm
        // to mean it's release number ("patch numbers" listed at
        // http://invisible-island.net/xterm/xterm.log.html),
        // and some applications use it as a feature check:
        // * tmux used to have a "xterm won't reach version 500 for a while so set that as the upper
        // limit" check,
        // and then check "xterm_version > 270" if rectangular area operations such as DECCRA could
        // be used.
      ...
      case 'm':
        ...
        // special keyboard
        // layouts (legacy and vt220). This is done to provide compatible keyboards for DEC VT220
        // and related
        ...
        // - Set it to 2 to force the modifier to be the second parameter if it would otherwise be
        // the first.
        ...
        // the first.
        // - Set it to 3 to mark the sequence with a ">" to hint that it is private.
        // If modifyFunctionKeys is zero, xterm uses Control- and Shift-modifiers to allow the user
        // to construct
        ...
        //
        // As a special case, legacy (when oldFunctionKeys is true) or vt220 (when sunKeyboard is
        // true)
        ...
        // as "2") when
        // modified by Control-, Alt- or Meta-modifiers. This feature does not apply to function
        // keys and
        ...
        mClient.logError(
            LOG_TAG, "(ignored) CSI > MODIFY RESOURCE: " + getArg0(-1) + " to " + getArg1(-1));
        break;
      default:
        parseArg(b);
        break;
  ...
  private void doEsc(int b) {
    switch (b) {
      ...
      case '6': // Back index (http://www.vt100.net/docs/vt510-rm/DECBI). Move left, insert blank
        ...
        if (mCursorCol > mLeftMargin) {
          ...
          mScreen.blockCopy(
              ...
              mTopMargin);
          mScreen.blockSet(
              mLeftMargin, mTopMargin, 1, rows, ' ', TextStyle.encode(mForeColor, mBackColor, 0));
        }
        break;
      ...
      case 'M': // "${ESC}M" - reverse index (RI).
        // http://www.vt100.net/docs/vt100-ug/chapter3.html: "Move the active position to the same
        // horizontal
        // position on the preceding line. If the active position is at the top margin, a scroll
        // down is performed".
  ...
  private void doCsi(int b) {
    switch (b) {
      ...
      case 'P': // "${CSI}{N}P" - delete ${N} characters (DCH).
        {
          // http://www.vt100.net/docs/vt510-rm/DCH: "If ${N} is greater than the number of
          // characters between the
          // cursor and the right margin, then DCH only deletes the remaining characters.
          // As characters are deleted, the remaining characters between the cursor and right margin
          // move to the left.
          // Character attributes move with the characters. The terminal adds blank spaces with no
          // visual character
      ...
      case 'T':
        if (mArgIndex == 0) {
          ...
          // http://vt100.net/docs/vt510-rm/SD: "N is the number of lines to move the user window up
          // in page
          // memory. N new lines appear at the top of the display. N old lines disappear at the
          // bottom of the
          ...
          // "${CSI}${func};${startx};${starty};${firstrow};${lastrow}T" - initiate highlight mouse
          // tracking.
      ...
      case 'c': // Primary Device Attributes (http://www.vt100.net/docs/vt510-rm/DA1) if argument is
        ...
        // for "vt420".
        // This is followed by a list of attributes which is probably unused by applications. Send
        // like xterm.
      ...
      case 'e': // Vertical Position Relative (VPR). From ISO-6429 (ECMA-48).
        ...
        break;
        // case 'f': "${CSI}${ROW};${COLUMN}f" - Horizontal and Vertical Position (HVP). Grouped
        // with case 'H'.
      ...
      case 'r': // "CSI${top};${bottom}r" - set top and bottom Margins (DECSTBM).
        {
          ...
          // The escape sequence numbers bottom 2..24, and so do we (because we use a zero based
          // numbering
    ...
    for (int i = 0; i <= mArgIndex; i++) {
      ...
      if (code == 0) { // reset
        ...
        mEffect &= ~(TextStyle.CHARACTER_ATTRIBUTE_BOLD | TextStyle.CHARACTER_ATTRIBUTE_DIM);
          ...
          == 23) { // not italic, but rarely used as such; clears standout with TERM=screen
        mEffect &= ~TextStyle.CHARACTER_ATTRIBUTE_ITALIC;
  ...
  private void doOscSetTextParameters(String bellOrStringTerminator) {
    ...
    switch (value) {
      ...
      case 2: // Change window title to T.
        ...
        break;
      case 4:
        // P s = 4 ; c ; spec → Change Color Number c to the color specified by spec. This can be a
        // name or RGB
        // specification as per XParseColor. Any number of c name pairs may be given. The color
        // numbers correspond
        // to the ANSI colors 0-7, their bright versions 8-15, and if supported, the remainder of
        // the 88-color or
        // 256-color table.
        // If a "?" is given rather than a name or RGB specification, xterm replies with a control
        // sequence of the
        // same form which can be used to set the corresponding color. Because more than one pair of
        // color number
        // and specification can be given in one control sequence, xterm can make more than one
        // reply.
        int colorIndex = -1;
      ...
      case 52: // Manipulate Selection Data. Skip the optional first selection parameter(s).
        ...
        break;
      case 104:
        // "104;$c" → Reset Color Number $c. It is reset to the color specified by the corresponding
        // X
        // resource. Any number of c parameters may be given. These parameters correspond to the
        // ANSI colors 0-7,
        // their bright versions 8-15, and if supported, the remainder of the 88-color or 256-color
        // table. If no
  ...
  private void setCursorPosition(int x, int y) {
    ...
    int newCol =
        Math.max(effectiveLeftMargin, Math.min(effectiveLeftMargin + x, effectiveRightMargin - 1));
  ...
  private void scrollDownOneLine() {
    ...
    if (mLeftMargin != 0 || mRightMargin != mColumns) {
      // Horizontal margin: Do not put anything into scroll history, just non-margin part of screen
      // up.
      mScreen.blockCopy(
          mLeftMargin,
          mTopMargin + 1,
