...
public final class TerminalRenderer {
  ...
  public final void render(
    ...
    for (int row = topRow; row < endRow; row++) {
      ...
      for (int column = 0; column < columns; ) {
        ...
        // Check if the measured text width for this code point is not the same as that expected by
        // wcwidth().
        // This could happen for some fonts which are not truly monospace, or for more exotic
        // characters such as
        ...
        while (currentCharIndex < charsUsedInLine && WcWidth.width(line, currentCharIndex) <= 0) {
          // Eat combining chars so that they are treated as part of the last non-combining code
          // point,
      ...
      final int charsSinceLastRun = currentCharIndex - lastRunStartIndex;
      int cursorColor =
          lastRunInsideCursor ? mEmulator.mColors.mCurrentColors[TextStyle.COLOR_INDEX_CURSOR] : 0;
      boolean invertCursorTextColor = false;
  ...
  private void drawTextRun(
    ...
    if ((effect & TextStyle.CHARACTER_ATTRIBUTE_INVISIBLE) == 0) {
      ...
      canvas.drawText(
          text, startCharIndex, runWidthChars, left, y - mFontLineSpacingAndAscent, mTextPaint);
    }
