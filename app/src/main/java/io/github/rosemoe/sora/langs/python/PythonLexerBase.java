...
public abstract class PythonLexerBase extends Lexer {
  ...
  private void setCurrentAndFollowingTokens() {
    this.curToken =
        this.ffgToken == null ? new CommonToken(super.nextToken()) : new CommonToken(this.ffgToken);
  ...
  private void insertLeadingIndentToken() {
    if (this.previousPendingTokenType == PythonLexer.WS) {
      ...
      if (this.getIndentationLength(prevToken.getText())
          != 0) { // there is an "indentation" before the first statement
        ...
        default:
          ...
          if (isLookingAhead) { // We're on whitespace(s) followed by a statement
            ...
            if (indentationLength != this.INVALID_LENGTH) {
              this.addPendingToken(this.curToken); // WS token
              this.insertIndentOrDedentToken(
                  indentationLength); // may insert INDENT token or DEDENT token(s)
            } else {
              this.reportError("inconsistent use of tabs and spaces in indentation");
            }
          } else { // We're at a newline followed by a statement (there is no whitespace before the
                   // statement)
  ...
  private void insertIndentOrDedentToken(final int indentLength) {
    ...
    if (indentLength > prevIndentLength) {
      ...
      while (indentLength
          < prevIndentLength) { // more than 1 DEDENT token may be inserted to the token stream
  ...
  private void
    ...
    if (this.curToken.getText().length() == line_joinFreeStringLiteral.length()) {
      ...
      this.hideAndAddPendingToken(
          originalSTRINGtoken); // add the original token to the hidden channel
      // this inserted hidden token allows to restore the original string literal with the
      // \<newline> escape sequences
    }
  }

  private void
      handleFSTRING_MIDDLE_token() { // replace the double braces '{{' or '}}' to single braces and
                                     // hide the second braces
    String fsMid = this.curToken.getText();
    fsMid =
        fsMid
            .replaceAll("\\{\\{", "{_")
            .replaceAll("}}", "}_"); // replace: {{ --> {_  and   }} --> }_
    String[] arrOfStr = fsMid.split("(?<=[{}])_"); // split by {_  or  }_
    for (String s : arrOfStr) {
      if (!s.isEmpty()) {
        this.createAndAddPendingToken(
            PythonLexer.FSTRING_MIDDLE, Token.DEFAULT_CHANNEL, s, this.ffgToken);
        String lastCharacter = s.substring(s.length() - 1);
        if ("{}".contains(lastCharacter)) {
          this.createAndAddPendingToken(
              PythonLexer.FSTRING_MIDDLE, Token.HIDDEN_CHANNEL, lastCharacter, this.ffgToken);
          // this inserted hidden token allows to restore the original f-string literal with the
          // double braces
        }
      }
    }
      ...
      switch (this.curToken.getType()) {
        ...
        case PythonLexer.COLON: // colon can only come from DEFAULT_MODE
          if (this.paren_or_bracket_openedStack.peek() == 0) {
            switch (_modeStack
                .peek()) { // check the previous lexer mode (the current is DEFAULT_MODE)
              ...
              case PythonLexer.SINGLE_QUOTE_FORMAT_SPECIFICATION_MODE:
                this.mode(
                    PythonLexer
                        .SINGLE_QUOTE_FORMAT_SPECIFICATION_MODE); // continue in format spec. mode
                break;
              ...
              case PythonLexer.DOUBLE_QUOTE_FORMAT_SPECIFICATION_MODE:
                this.mode(
                    PythonLexer
                        .DOUBLE_QUOTE_FORMAT_SPECIFICATION_MODE); // continue in format spec. mode
                break;
            }
          }
          break;
  ...
  private void insertTrailingTokens() {
    switch (this.lastPendingTokenTypeFromDefaultChannel) {
      ...
      default:
        ...
        this.createAndAddPendingToken(
            PythonLexer.NEWLINE, Token.DEFAULT_CHANNEL, null, this.ffgToken); // ffgToken is EOF
    }
  ...
  private void createAndAddPendingToken(
      final int type, final int channel, final String text, Token baseToken) {
  ...
  private void addPendingToken(final Token token) {
    // save the last pending token type because the pendingTokens linked list can be empty by the
    // nextToken()
  ...
  private int getIndentationLength(
      final String textWS) { // the textWS may contain spaces, tabs or form feeds
  ...
  private void reportError(final String errMsg) {
    ...
    this.createAndAddPendingToken(
        PythonLexer.ERROR_TOKEN, Token.DEFAULT_CHANNEL, this.ERR_TXT + errMsg, this.ffgToken);
  }
