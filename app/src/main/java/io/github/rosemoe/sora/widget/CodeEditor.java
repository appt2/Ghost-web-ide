...
public class CodeEditor extends View
		implements ContentListener, TextAnalyzer.Callback, FormatThread.FormatResultReceiver, LineRemoveListener {
		...
		if (cur.isSelected()) {
			...
			if (mProps.deleteEmptyLineFast
				...
				if (inLeading) {
					...
					if (mProps.deleteMultiSpaces != 1 && col > 0 && mText.charAt(line, col - 1) == ' ') {
						mText.delete(line,
								Math.max(0, col
										- (mProps.deleteMultiSpaces == -1 ? getTabWidth() : mProps.deleteMultiSpaces)),
								line, col);
						return;
					}
				}
			...
			if (begin == end) {
				if (cur.getLeftLine() > 0) {
					mText.delete(cur.getLeftLine() - 1, mText.getColumnCount(cur.getLeftLine() - 1), cur.getLeftLine(),
							0);
				}
			} else {
  ...
  public float measureTextRegionOffset() {
	...
	private void initialize() {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			...
			} catch (SecurityException | InvocationTargetException | NoSuchMethodException | IllegalAccessException
		...
		mDpUnit = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 10, Resources.getSystem().getDisplayMetrics())
				/ 10F;
	...
	protected void drawView(Canvas canvas) {
		...
		if (lineNumberNotPinned) {
			drawLineNumberBackground(canvas, offsetX, lineNumberWidth + mDividerMargin,
			...
			for (int i = 0; i < postDrawCurrentLines.size(); i++) {
				drawRowBackground(canvas, currentLineBgColor, (int) postDrawCurrentLines.get(i),
						(int) (textOffset - mDividerMargin));
			}
			drawDivider(canvas, offsetX + lineNumberWidth + mDividerMargin,
			...
			if (firstLn != null && firstLn.value != -1) {
				...
				if (postDrawLineNumbers.size() == 0
					...
					y = (getRowBottom(0) + getRowTop(0)) / 2f
							- (mLineNumberMetrics.descent - mLineNumberMetrics.ascent) / 2f - mLineNumberMetrics.ascent;
				} else {
				...
				case RIGHT:
					...
					break;
				case CENTER:
					canvas.drawText(Integer.toString(firstLn.value + 1),
							offsetX + (lineNumberWidth + mDividerMargin) / 2f, y, mPaintOther);
				}
			}
			for (int i = 0; i < postDrawLineNumbers.size(); i++) {
				long packed = postDrawLineNumbers.get(i);
				drawLineNumber(canvas, IntPair.getFirst(packed), IntPair.getSecond(packed), offsetX, lineNumberWidth,
						lineNumberColor);
			}
		}
		...
		if (isLineNumberEnabled() && !lineNumberNotPinned) {
			drawLineNumberBackground(canvas, 0, lineNumberWidth + mDividerMargin,
					color.getColor(EditorColorScheme.LINE_NUMBER_BACKGROUND));
			int lineNumberColor = mColors.getColor(EditorColorScheme.LINE_NUMBER);
			int currentLineBgColor = mColors.getColor(EditorColorScheme.CURRENT_LINE);
			if (mCursorAnimator.isRunning()) {
			...
			for (int i = 0; i < postDrawLineNumbers.size(); i++) {
				long packed = postDrawLineNumbers.get(i);
				drawLineNumber(canvas, IntPair.getFirst(packed), IntPair.getSecond(packed), 0, lineNumberWidth,
						lineNumberColor);
			}
		}
	...
	protected boolean shouldInitializeNonPrintable() {
		return clearFlag(clearFlag(mNonPrintableOptions, FLAG_DRAW_WHITESPACE_FOR_EMPTY_LINE),
				FLAG_DRAW_TAB_SAME_AS_SPACE) != 0;
	}
	...
	protected void updateLineDisplayList(RenderNode renderNode, int line, List<Span> spans) {
		...
		while (columnCount > span.column) {
			...
			if (backgroundColorId != 0) {
				drawRowRegionBackground(canvas, paintingOffset, row, 0, columnCount, paintStart, paintEnd,
						mColors.getColor(backgroundColorId), line);
			}

			// Draw text
			drawRegionText(canvas, paintingOffset, getRowBaseline(row), line, paintStart, paintEnd, span.column,
					spanEnd, columnCount, mColors.getColor(span.getForegroundColorId()));

			// Draw strikethrough
			if ((span.problemFlags & Span.FLAG_DEPRECATED) != 0 || TextStyle.isStrikeThrough(span.style)) {
				mPaintOther.setColor(0xFF281D1B);
				canvas.drawLine(paintingOffset, getRowTop(row) + getRowHeight() / 2f, paintingOffset + width,
						getRowTop(row) + getRowHeight() / 2f, mPaintOther);
			}
	...
	protected void drawRows(Canvas canvas, float offset, LongArrayList postDrawLineNumbers,
			List<DrawCursorTask> postDrawCursor, LongArrayList postDrawCurrentLines, MutableInt requiredFirstLn) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && !isWordwrap() && canvas.isHardwareAccelerated()
		...
		for (int row = firstVis; row <= getLastVisibleRow() && rowIterator.hasNext(); row++) {
			...
			int lastVisibleChar = (int) findFirstVisibleChar(offset2 + getWidth(), firstVisibleChar + 1,
					rowInf.endColumn, rowInf.startColumn, mBuffer, line)[0];
			...
			if (!matchedPositions.isEmpty()) {
				for (int position : matchedPositions) {
					drawRowRegionBackground(canvas, paintingOffset, row, firstVisibleChar, lastVisibleChar, position,
							position + mSearcher.mSearchText.length(),
							mColors.getColor(EditorColorScheme.MATCHED_TEXT_BACKGROUND), line);
				}
			}
			...
			if (mCursor.isSelected() && line >= mCursor.getLeftLine() && line <= mCursor.getRightLine()) {
				...
				if (mText.getColumnCount(line) == 0 && line != mCursor.getRightLine()) {
					...
					drawRowRegionBackground(canvas, paintingOffset, row, firstVisibleChar, lastVisibleChar,
							selectionStart, selectionEnd, mColors.getColor(EditorColorScheme.SELECTED_TEXT_BACKGROUND),
							line);
				}
			}
		}
		...
		for (int row = firstVis; row <= getLastVisibleRow() && rowIterator.hasNext(); row++) {
			...
			// Draw text here
			if (!mHardwareAccAllowed || !canvas.isHardwareAccelerated() || isWordwrap()
				...
				while (spanOffset + 1 < spans.size()) {
					if (spans.get(spanOffset + 1).column <= firstVisibleChar) {
						...
						if (span.problemFlags > 0 && Integer.highestOneBit(span.problemFlags) != Span.FLAG_DEPRECATED) {
							...
							if (isWordwrap()) {
								lineWidth = measureText(mBuffer, Math.max(firstVisibleChar, span.column),
										spanEnd - Math.max(firstVisibleChar, span.column), line) + phi;
							} else {
				...
				while (lastVisibleChar > span.column) {
					...
					if (backgroundColorId != 0) {
						drawRowRegionBackground(canvas, paintingOffset, row, firstVisibleChar, lastVisibleChar,
								paintStart, paintEnd, mColors.getColor(backgroundColorId), line);
					}

					// Draw text
					drawRegionText(canvas, paintingOffset, getRowBaseline(row) - getOffsetY(), line, paintStart,
							paintEnd, span.column, spanEnd, columnCount, mColors.getColor(span.getForegroundColorId()));
					...
					if ((span.problemFlags & Span.FLAG_DEPRECATED) != 0 || TextStyle.isStrikeThrough(styleBits)) {
						...
						canvas.drawLine(paintingOffset, getRowTop(row) + getRowHeight() / 2f - getOffsetY(),
								paintingOffset + width, getRowTop(row) + getRowHeight() / 2f - getOffsetY(),
								mPaintOther);
					}
					...
					if (span.problemFlags > 0 && Integer.highestOneBit(span.problemFlags) != Span.FLAG_DEPRECATED) {
						...
						if (color != 0 && span.column >= 0 && spanEnd - span.column >= 0) {
							...
							float lineWidth;
							if (isWordwrap()) {
								startOffset = measureTextRegionOffset() + measureText(mBuffer, firstVisibleChar,
										Math.max(0, span.column - firstVisibleChar), line) - getOffsetX();
								lineWidth = measureText(mBuffer, Math.max(firstVisibleChar, span.column),
										spanEnd - Math.max(firstVisibleChar, span.column), line) + phi;
							} else {
								startOffset = measureTextRegionOffset() + measureText(mBuffer, 0, span.column, line)
										- getOffsetX();
								lineWidth = measureText(mBuffer, span.column, spanEnd - span.column, line) + phi;
							}
							...
							int waveCount = (int) Math.ceil(lineWidth / waveLength);
							for (int i = 0; i < waveCount; i++) {
								mPath.quadTo(waveLength * i + waveLength / 4, amplitude,
										waveLength * i + waveLength / 2, 0);
								mPath.quadTo(waveLength * i + waveLength * 3 / 4, -amplitude,
										waveLength * i + waveLength, 0);
							}
							phi = waveLength - (waveCount * waveLength - lineWidth);
							// Draw path
							mPaint.setStrokeWidth(getDpUnit() * 1.8f);
			...
			if (circleRadius != 0f && (leadingWhitespaceEnd != columnCount
				...
				if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_LEADING) != 0) {
					drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar, 0,
							leadingWhitespaceEnd, circleRadius);
				}
				if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_INNER) != 0) {
					drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
							leadingWhitespaceEnd, trailingWhitespaceStart, circleRadius);
				}
				if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_TRAILING) != 0) {
					drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
							trailingWhitespaceStart, columnCount, circleRadius);
				}
				if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_IN_SELECTION) != 0 && mCursor.isSelected()
						&& line >= mCursor.getLeftLine() && line <= mCursor.getRightLine()) {
					int selectionStart = 0;
					int selectionEnd = columnCount;
					if (line == mCursor.getLeftLine()) {
						selectionStart = mCursor.getLeftColumn();
					...
					if ((mNonPrintableOptions & 0b1110) == 0) {
						drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
								selectionStart, selectionEnd, circleRadius);
					} else {
						if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_LEADING) == 0) {
							drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
									selectionStart, Math.min(leadingWhitespaceEnd, selectionEnd), circleRadius);
						}
						if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_INNER) == 0) {
							drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
									Math.max(leadingWhitespaceEnd, selectionStart),
									Math.min(trailingWhitespaceStart, selectionEnd), circleRadius);
						}
						if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_TRAILING) == 0) {
							drawWhitespaces(canvas, paintingOffset, line, row, firstVisibleChar, lastVisibleChar,
									Math.max(trailingWhitespaceStart, selectionStart), selectionEnd, circleRadius);
						}
					}
				}
			}
			...
			if (mCursor.isSelected()) {
				if (mCursor.getLeftLine() == line
					...
					float centerX = paintingOffset
							+ measureText(mBuffer, firstVisibleChar, mCursor.getLeftColumn() - firstVisibleChar, line);
					postDrawCursor.add(new DrawCursorTask(centerX, getRowBottom(row) - getOffsetY(),
							SelectionHandleStyle.HANDLE_TYPE_LEFT, mLeftHandle));
				}
				if (mCursor.getRightLine() == line
						&& isInside(mCursor.getRightColumn(), firstVisibleChar, lastVisibleChar, line)) {
					float centerX = paintingOffset
							+ measureText(mBuffer, firstVisibleChar, mCursor.getRightColumn() - firstVisibleChar, line);
					postDrawCursor.add(new DrawCursorTask(centerX, getRowBottom(row) - getOffsetY(),
							SelectionHandleStyle.HANDLE_TYPE_RIGHT, mRightHandle));
				}
				...
				float centerX = paintingOffset
						+ measureText(mBuffer, firstVisibleChar, mCursor.getLeftColumn() - firstVisibleChar, line);
				postDrawCursor.add(new DrawCursorTask(centerX, getRowBottom(row) - getOffsetY(),
						mEventHandler.shouldDrawInsertHandle() ? SelectionHandleStyle.HANDLE_TYPE_INSERT
	...
	/** Draw non-printable characters */
	protected void drawWhitespaces(Canvas canvas, float offset, int line, int row, int rowStart, int rowEnd, int min,
			int max, float circleRadius) {
	...
	protected long findLeadingAndTrailingWhitespacePos(int line) {
		...
		if (leading != column
				&& (mNonPrintableOptions & (FLAG_DRAW_WHITESPACE_INNER | FLAG_DRAW_WHITESPACE_TRAILING)) != 0) {
	 ...
	 */
	protected void drawRowRegionBackground(Canvas canvas, float paintingOffset, int row, int firstVis, int lastVis,
			int highlightStart, int highlightEnd, int color, int line) {
	 ...
	 */
	protected void drawRegionText(Canvas canvas, float offsetX, float baseline, int line, int startIndex, int endIndex,
			int contextStart, int contextEnd, int columnCount, int color) {
		boolean hasSelectionOnLine = mCursor.isSelected() && line >= mCursor.getLeftLine()
		...
		if (hasSelectionOnLine && mColors.getColor(EditorColorScheme.TEXT_SELECTED) != 0) {
			if (endIndex <= selectionStart || startIndex >= selectionEnd) {
				drawText(canvas, mBuffer, startIndex, endIndex - startIndex, contextStart, contextCount, offsetX,
						baseline, line);
			} else {
				if (startIndex <= selectionStart) {
					if (endIndex >= selectionEnd) {
						...
						drawText(canvas, mBuffer, startIndex, selectionStart - startIndex, contextStart, contextCount,
								offsetX, baseline, line);
						...
						drawText(canvas, mBuffer, selectionStart, selectionEnd - selectionStart, contextStart,
								contextCount, offsetX + deltaX, baseline, line);
						deltaX += measureText(mBuffer, selectionStart, selectionEnd - selectionStart, line);
						// selectionEnd - endIndex
						mPaint.setColor(color);
						drawText(canvas, mBuffer, selectionEnd, endIndex - selectionEnd, contextStart, contextCount,
								offsetX + deltaX, baseline, line);
					} else {
						// Two regions
						// startIndex - selectionStart
						drawText(canvas, mBuffer, startIndex, selectionStart - startIndex, contextStart, contextCount,
						...
						mPaint.setColor(mColors.getColor(EditorColorScheme.TEXT_SELECTED));
						drawText(canvas, mBuffer, selectionStart, endIndex - selectionStart, contextStart, contextCount,
								offsetX + measureText(mBuffer, startIndex, selectionStart - startIndex, line), baseline,
								line);
					}
				} else {
					...
					if (endIndex > selectionEnd) {
						...
						// selectionEnd - endIndex
						drawText(canvas, mBuffer, selectionEnd, endIndex - selectionEnd, contextStart, contextCount,
								offsetX + measureText(mBuffer, startIndex, selectionEnd - startIndex, line), baseline,
								line);
						// startIndex - selectionEnd
						mPaint.setColor(mColors.getColor(EditorColorScheme.TEXT_SELECTED));
						drawText(canvas, mBuffer, startIndex, selectionEnd - startIndex, contextStart, contextCount,
								offsetX, baseline, line);
					} else {
						// One region
						mPaint.setColor(mColors.getColor(EditorColorScheme.TEXT_SELECTED));
						drawText(canvas, mBuffer, startIndex, endIndex - startIndex, contextStart, contextCount,
								offsetX, baseline, line);
					}
				}
			}
		} else {
			drawText(canvas, mBuffer, startIndex, endIndex - startIndex, contextStart, contextCount, offsetX, baseline,
					line);
		}
	}
		...
		for (int curr = mm; curr < blocks.size(); curr++) {
			...
			if (hasVisibleRegion(block.startLine, block.endLine, first, last)) {
				try {
					...
					mRect.bottom = Math.min(getHeight(),
					...
					drawColor(canvas, mColors.getColor(
							curr == cursorIdx ? EditorColorScheme.BLOCK_LINE_CURRENT : EditorColorScheme.BLOCK_LINE),
							mRect);
	...
	protected void drawScrollBarVertical(Canvas canvas) {
		...
		drawColor(canvas,
				mColors.getColor(mEventHandler.holdVerticalScrollBar() ? EditorColorScheme.SCROLL_BAR_THUMB_PRESSED
	...
	protected void drawScrollBarHorizontal(Canvas canvas) {
		...
		drawColor(canvas,
				mColors.getColor(mEventHandler.holdHorizontalScrollBar() ? EditorColorScheme.SCROLL_BAR_THUMB_PRESSED
						: EditorColorScheme.SCROLL_BAR_THUMB),
				mRect);
	}
	...
	protected void drawSelectionOnAnimation(Canvas canvas) {
		...
		if (mEventHandler.shouldDrawInsertHandle()) {
			mHandleStyle.draw(canvas, SelectionHandleStyle.HANDLE_TYPE_INSERT, centerX, mRect.bottom, getRowHeight(),
					mColors.getColor(EditorColorScheme.SELECTION_HANDLE), mInsertHandle);
		}
	}
	 ...
	 */
	@SuppressLint("NewApi")
	protected void drawText(Canvas canvas, ContentLine line, int index, int count, int contextStart, int contextCount,
			float offX, float offY, int lineNumber) {
	...
	@UnsupportedUserUsage
	public float[] findFirstVisibleChar(float target, int start, int end, int contextStart, ContentLine line,
			int lineNumber) {
		if (start >= end) {
	...
	protected float updateCursorAnchor() {
		...
		builder.setInsertionMarkerLocation(x, getRowTop(l) - getOffsetY(), getRowBaseline(l) - getOffsetY(),
	...
	public void ensurePositionVisible(int line, int column) {
		...
		if (animation) {
			...
			getScroller().startScroll(getOffsetX(), getOffsetY(), (int) (targetX - getOffsetX()),
			...
			getScroller().startScroll(getOffsetX(), getOffsetY(), (int) (targetX - getOffsetX()),
					(int) (targetY - getOffsetY()), 0);
		}

		dispatchEvent(new ScrollEvent(this, getOffsetX(), getOffsetY(), (int) targetX, (int) targetY,
			...
			public boolean onActionItemClicked(final ActionMode am, MenuItem p2) {
				...
				case 3:
					...
					new MaterialAlertDialogBuilder(getContext())
							.setTitle(replaceAll ? R.string.replaceAll : R.string.replace).setView(et)
	...
	public void moveSelectionDown() {
		if (mLockedSelection == null) {
			...
			setSelectionRegion(mLockedSelection.line, mLockedSelection.column, IntPair.getFirst(pos),
					IntPair.getSecond(pos), false);
	...
	public void moveSelectionEnd() {
		if (mLockedSelection == null) {
			...
			setSelectionRegion(mLockedSelection.line, mLockedSelection.column, line, getText().getColumnCount(line),
					false);
			ensureSelectingTargetVisible();
		}
	}
	...
	public void setSelectionRegion(int lineLeft, int columnLeft, int lineRight, int columnRight,
		...
		if (start > end) {
			...
			Log.w(LOG_TAG,
					"setSelectionRegion() error: start > end:start = " + start + " end = " + end + " lineLeft = "
							+ lineLeft + " columnLeft = " + columnLeft + " lineRight = " + lineRight + " columnRight = "
							+ columnRight);
			return;
		}
		mCursorAnimator.cancel();
	...
	public void copyText() {
		try {
			if (mCursor.isSelected()) {
				String clip = getText().subContent(mCursor.getLeftLine(), mCursor.getLeftColumn(),
						mCursor.getRightLine(), mCursor.getRightColumn()).toString();
	...
	public void setText(@Nullable CharSequence text) {
		...
		dispatchEvent(new ContentChangeEvent(this, ContentChangeEvent.ACTION_SET_NEW_TEXT, new CharPosition(),
				mText.getIndexer().getCharPosition(getLineCount() - 1, mText.getColumnCount(getLineCount() - 1)),
				mText));
	...
	protected void updateSelection() {
		...
		mInputMethodManager.updateSelection(this, mCursor.getLeft(), mCursor.getRight(), candidatesStart,
				candidatesEnd);
	}
	...
	public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
		...
		outAttrs.inputType = mInputType != 0 ? mInputType
					...
					for (NewlineHandler handler : handlers) {
						if (handler != null) {
							if (handler.matchesRequirement(beforeText, afterText)) {
								try {
									NewlineHandler.HandleResult result = handler.handleNewline(beforeText, afterText,
											getTabWidth());
									if (result != null) {
										...
										if (delta != 0) {
											...
											CharPosition charPosition = getCursor().getIndexer()
				...
				if (event.isPrintingKey() && isEditable()) {
					...
					if (replacement == null || replacement == SymbolPairMatch.Replacement.NO_REPLACEMENT
						...
						if (getCursor().isSelected()
							...
							// insert left
							getText().insert(getCursor().getLeftLine(), getCursor().getLeftColumn(),
									autoSurroundPair[0]);
							// insert right
							getText().insert(getCursor().getRightLine(), getCursor().getRightColumn(),
									autoSurroundPair[1]);
	...
	protected void onSizeChanged(int w, int h, int oldWidth, int oldHeight) {
		...
		if (isWordwrap() && w != oldWidth) {
			...
			mEventHandler.scrollBy(getOffsetX() > getScrollMaxX() ? getScrollMaxX() - getOffsetX() : 0,
	...
	public void afterInsert(Content content, int startLine, int startColumn, int endLine, int endColumn,
		...
		if (isSpanMapPrepared(true, endLine - startLine)) {
			if (startLine == endLine) {
				SpanMapUpdater.shiftSpansOnSingleLineInsert(mSpanner.getResult().getSpanMap(), startLine, startColumn,
						endColumn);
			} else {
				SpanMapUpdater.shiftSpansOnMultiLineInsert(mSpanner.getResult().getSpanMap(), startLine, startColumn,
						endLine, endColumn);
			}
		}
		...
		if (isAutoCompletionEnabled()) {
			if ((mConnection.mComposingLine == -1 || mCompletionOnComposing) && endColumn != 0
					&& startLine == endLine) {
				int end = endColumn;
				while (endColumn > 0) {
		...
		dispatchEvent(new ContentChangeEvent(this, ContentChangeEvent.ACTION_INSERT,
				mText.getIndexer().getCharPosition(startLine, startColumn),
				mText.getIndexer().getCharPosition(endLine, endColumn), insertedContent));
	}
	...
	public void afterDelete(Content content, int startLine, int startColumn, int endLine, int endColumn,
		...
		if (isSpanMapPrepared(false, endLine - startLine)) {
			if (startLine == endLine) {
				SpanMapUpdater.shiftSpansOnSingleLineDelete(mSpanner.getResult().getSpanMap(), startLine, startColumn,
						endColumn);
			} else {
				SpanMapUpdater.shiftSpansOnMultiLineDelete(mSpanner.getResult().getSpanMap(), startLine, startColumn,
						endLine, endColumn);
			}
		}
	...
	private class DrawCursorTask {
		...
		void execute(Canvas canvas) {
			...
			if (!descriptor.position.isEmpty()) {
				if ((mEventHandler.holdInsertHandle() && handleType == SelectionHandleStyle.HANDLE_TYPE_INSERT)
					...
					x = mEventHandler.mMotionX
							+ (descriptor.alignment != SelectionHandleStyle.ALIGN_CENTER ? descriptor.position.width()
									: 0) * (descriptor.alignment == SelectionHandleStyle.ALIGN_LEFT ? 1 : -1);
					y = mEventHandler.mMotionY - descriptor.position.height() * 2 / 3f;
				}
			}

			if (handleType != SelectionHandleStyle.HANDLE_TYPE_UNDEFINED || mCursorBlink.visibility
			...
			if (handleType != SelectionHandleStyle.HANDLE_TYPE_UNDEFINED) {
				mHandleStyle.draw(canvas, handleType, x, y, getRowHeight(),
						mColors.getColor(EditorColorScheme.SELECTION_HANDLE), descriptor);
